  ...
  const save = async () => {
    try {
      ...
      if (editing) {
        ...
        await pushCategory({ id: editing.id, name, color });
      } else {
        const id = Date.now(); // ID único compatible Firebase
        await insertCategory(name, color);
        await pushCategory({ id, name, color });
      }
